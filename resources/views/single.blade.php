...
@section('content')
    ...
    <div class="header" id="{{$user->id}}">
        <div class="cover" id="coverPhoto" style="background: {{$user->cover ? 'url(\' ' . asset('/storage/covers/' . $user->cover) . ' \')': '#1DA1F2'}}">
        ...
        <div class="user">
            <div class="container">
                @if(Auth::user()->id === $user->id)
                    <div class="profile-pic" {{!is_null($user->photo) ? 'style=background:url(\'' . asset('storage/photos/' . $user->photo) . '\')' : ''}} id='dropdownMenuPhoto' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false' title="Update your profile picture">
                ...
                @else
                    <div class="profile-pic" {{is_null($user->photo) ? "id=dropdownMenuPhoto data-toggle=dropdown aria-haspopup=true aria-expanded=false": 'style=background:url(\'' . asset('storage/photos/' . $user->photo) . '\')'}}></div>
                @endif
                <ul class="stats">
            ...
            <div class="row">
                ...
                <div class="col-xs-12 col-sm-12 col-md-9 col-lg-6 content">
                    ...
                    <div class="posts" id="{{$tweet->id}}">
                        ...
                        <div class="post">
                            ...
                            <div class="postContent">
                                <a href="{{route('user.show', $tweet->user->username)}}" class="user">
                                    <img src="{{$tweet->user->photo ? asset('/storage/photos/' . $tweet->user->photo) : env('DEFAULT_PHOTO')}}">
                                </a>
                                <div class="data">
                        ...
                        <div class="comment" v-for="comment in comments" :id="comment.id">
                            <div class="photo">
                                <a :href="'{{route('user.show', '/')}}/' + comment.user.username">
                                    <img :src="(comment.user.photo) ? '{{asset('/storage/photos')}}' + '/' + comment.user.photo: 'https://abs.twimg.com/sticky/default_profile_images/default_profile_normal.png'">
                                </a>
                            </div>
                        ...
                        <div class="publish">
                            <div class="photo">
                                <img src="{{ !is_null(Auth::user()->photo) ? asset('/storage/photos/' . Auth::user()->photo) : env('DEFAULT_PHOTO')}}">
                            </div>
